...
```
- ID选择器：`#id`，一个页面里只能有一个特定名字的id。用来选择被标记为特定idName的元素，最佳实践里要尽量减少对id的使用，因为这不利于元素的重利用。规模较大时，id的数量变多会导致重构效率非常低下，在选择元素时，如果追踪的是特定相对位置或者某些有规律可循的定位可使用**[结构伪类选择器](#structure_s)**(点击跳转)
